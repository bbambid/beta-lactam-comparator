...
(function(){
  ...
  // 1) Clavamox pediatric combination dry syrup (AMPC/CVA)
  DB.clav={
    ...
    adult:null,
    source:"PMDA/MHLW クラバモックス小児用配合ドライシロップ",
    warning:"食直前投与。配合剤のため、画面の成分量はAMPC+CVA合計量です。"
  };

  // 2) Minocycline granules 2%
  DB.mino={
    ...
    adult:[200,200],
    source:"PMDA ミノマイシン顆粒2% 2026年6月電子添文",
    warning:"小児、特に歯牙形成期の8歳未満では歯牙着色等のため、他剤が使用できない・無効の場合にのみ適用を考慮。"
  };

  // 3) Cyproheptadine (Periactin) syrup
  ...
  DB.cypro={
    ...
    indications:{general:{label:"アレルギー性疾患等",lo:(w,a)=>cyproDose(a),hi:(w,a)=>3*cyproDose(a),freq:[1,2,3],perDoseLo:(w,a)=>cyproDose(a),perDoseHi:(w,a)=>cyproDose(a),desc:"電子添文のAugsberger式参考量：2～3歳1.2mg/回、4～6歳1.6mg/回、7～9歳2mg/回、10～12歳2.6mg/回。通常1日1～3回。"}},
    adult:[4,12],
    source:"PMDA ペリアクチンシロップ0.04%電子添文"
  };

  // 4) Salbutamol (Ventolin) syrup
  DB.salbu={
    ...
    adult:[12,12],
    source:"PMDA/MHLW ベネトリンシロップ0.04%"
  };

  // 5) Loperamide pediatric granules 0.05%
  DB.lope={
    ...
    adult:[1,2],
    source:"PMDA/MHLW ロペミン小児用細粒0.05%",
    warning:"6か月未満は禁忌。6か月以上2歳未満は原則として投与を避け、治療上やむを得ない場合に限る。"
  };

  // 6) Diazepam suppositories (Diaup)
  DB.diaSupp={
    ...
    indications:{seizure:{label:"熱性けいれん／てんかんのけいれん発作",lo:(w,a)=>0,hi:(w,a)=>1*w,max:(w,a)=>1*w,freq:[1,2],perDoseLo:(w,a)=>0.4*w,perDoseHi:(w,a)=>0.5*w,desc:"1回0.4～0.5mg/kgを1日1～2回。1日1mg/kgを超えない。"}},
    adult:null,
    source:"PMDA/MHLW ダイアップ坐剤"
  };

  // 7) Magnesium oxide (Magmitt)
  DB.mgo={
    ...
    adult:[500,2000],
    source:"PMDA マグミット細粒83%／錠 2025年小児用量追加",
    warning:"高Mg血症に注意。腎機能・長期投与・高用量では特に慎重に。"
  };

  // 8) Medicon combination syrup; output active mg is dextromethorphan component
  DB.mediconCombo={
    ...
    adult:[45,60],
    source:"厚労省 臨床研究情報ポータル／メジコン配合シロップ"
  };

  // 9) Acetaminophen suppository formulations (same dose logic as existing APAP)
  Object.assign(DB.apap.products,{
    supp50:{label:"アンヒバ坐剤小児用50mg",unit:"個",mgPerUnit:50,defaultAmount:1},
    supp100:{label:"アンヒバ坐剤小児用100mg",unit:"個",mgPerUnit:100,defaultAmount:1},
    supp200:{label:"アンヒバ坐剤小児用200mg",unit:"個",mgPerUnit:200,defaultAmount:1}
  });

  // 10) Domperidone suppositories; separate key because dose logic differs from oral DS
  ...
  DB.dompSupp={
    ...
    indications:{general:{label:"小児の悪心・嘔吐等",lo:(w,a)=>2*dompSuppDose(a),hi:(w,a)=>3*dompSuppDose(a),freq:[2,3],perDoseLo:(w,a)=>dompSuppDose(a),perDoseHi:(w,a)=>dompSuppDose(a),desc:"3歳未満：1回10mg、3歳以上：1回30mgを1日2～3回直腸内投与。"}},
    adult:null,
    source:"PMDA ナウゼリン坐剤10/30 2026年8月電子添文"
  };

  const options=[
    ["clav","クラバモックス（AMPC/CVA）"],["mino","ミノサイクリン"],["cypro","シプロヘプタジン（ペリアクチン）"],["salbu","サルブタモール（ベネトリン）"],["lope","ロペラミド（ロペミン）"],["diaSupp","ジアゼパム坐剤（ダイアップ）"],["mgo","酸化マグネシウム（マグミット）"],["mediconCombo","メジコン配合シロップ"],["dompSupp","ドンペリドン坐剤（ナウゼリン）"]
  ];
  const sel=$("drug");
  ...
  loadDrug=function(reset=true){
    ...
    else if(k==="lope"||k==="diaSupp"||k==="dompSupp") $("freq").innerHTML='<option value="2">分2</option><option value="3">分3</option>'+(k==="diaSupp"?'<option value="1">1回</option>':'');
