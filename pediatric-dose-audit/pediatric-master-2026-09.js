 ...
 */
window.PEDIATRIC_RELEASE=Object.freeze({version:'1.1.1',appUpdated:'2026-09-13',dataBaseline:'2026-09-07'});
...
+(function(){
  ...
  function itemHtml(item){
    ...
    const sensory=chips.length?chips.map(([k,v])=>'<span class="palatability-chip"><b>'+k+'</b> '+esc(v)+'</span>').join(""):'<span class="palatability-chip muted">公式一次資料に具体的な味・香りの直接記載なし</span>';
    const match=formulationMatch(item);
    return '<details class="palatability-item"><summary><span class="palatability-product"><b>'+esc(item.product)+'</b><span>'+esc(item.manufacturer)+'／'+esc(item.formulation)+'</span></span><span class="palatability-status '+(chips.length?'verified':'no-direct')+'">'+(chips.length?'直接記載確認':'具体記載なし')+'</span>'+(match?'<span class="palatability-match">選択規格</span>':'')+'</summary><div class="palatability-body"><div class="palatability-chips">'+sensory+'</div>'+(item.instruction?'<div class="palatability-sub"><b>服薬時：</b>'+esc(item.instruction)+'</div>':'')+foodHtml(item)+(item.auditNotes.length?'<div class="palatability-audit-note"><b>監査補正：</b>'+esc(item.auditNotes.join('／'))+'</div>':'')+'<div class="palatability-sources">'+sourceHtml(item)+'</div></div></details>';
  }
  function draw(){
